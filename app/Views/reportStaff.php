<main>
  <section>
    <h2>Relatório de Prevenção</h2>
    <div class="col-4 offset-4">
      <form action="<?= url_to('postReportStaff') ?>" method="post">
        <?php helper('form'); ?>
        <?= session()->getFlashdata('error') ?>
        <?= validation_list_errors() ?>
        <?= csrf_field() ?>
        <div class="mb-3">
          <label for="staff" class="form-label">Chave:</label>
          <select name="staff" id="staff" class="form-select">
            <option value="0" selected disabled>Escolha um staff</option>
            <?php
              foreach ($staff as $key => $value) { ?>
                <option value="<?= $value['id'] ?>"><?=ucfirst($value['name'])?></option>
              <?php }; ?>
          </select>
          <small><?= session()->get('err')['staff'] ?? '' ?></small>
        </div>
        <div class="mb-3">
